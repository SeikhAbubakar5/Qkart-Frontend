...
const Checkout = () => {
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      // TODO: CRIO_TASK_MODULE_CHECKOUT - Delete selected address from the backend and display the latest list of addresses
        ...
        setAddresses({...addresses,all:response.data})
        //success message
        enqueueSnackbar("Deleted",{varient:"success"})
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
  ...
  const performCheckout = async (token, items, addresses) => {
    let isValidREquest=validateRequest(items,addresses);
    if(isValidREquest){
      try {
        ...
        if(res.data.success){
          let walletBal=localStorage.getItem('balance');
          let remainingBal=walletBal-getTotalCartValue(items);
          localStorage.setItem("balance",remainingBal);
          history.push("/thanks")
